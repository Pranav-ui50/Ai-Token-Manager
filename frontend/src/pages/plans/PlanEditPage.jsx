...
const PlanEditPage = () => {
        ...
        if (planResponse.success) {
          ...
          setFormData({
            ...
            pricingModel: {
              ...
              usageBased: plan.pricingModel?.usageBased || {
                pricePerToken: 0,
                pricePerRequest: 0,
                includedTokens: 0,
                includedRequests: 0,
            ...
            credits: {
              ...
              creditPricing: plan.credits?.creditPricing || {
                pricePerCredit: 0,
                bulkDiscounts: [],
                creditPacks: []
              },
              autoRecharge: plan.credits?.autoRecharge || {
                enabled: false,
                threshold: 100,
                rechargeAmount: 500
              }
            ...
            costs: {
              fixedCostsPerMonth: plan.costs?.fixedCostsPerMonth || 0,
              variableCostPercentage: plan.costs?.variableCostPercentage || 2.9
            },
            settings: {
  ...
  const toggleFeature = (featureId) => {
    setFormData(prev => {
      const isSelected = prev.features.some(f => f.feature === featureId || f.feature?._id === featureId);
      if (isSelected) {
        return {
          ...prev,
          features: prev.features.filter(f => (f.feature?._id || f.feature) !== featureId)
        };
      } else {
        return {
          ...prev,
  ...
  const updateFeatureLimits = (featureId, field, value) => {
    setFormData(prev => ({
      ...
      features: prev.features.map(f =>
        (f.feature?._id || f.feature) === featureId
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await planApi.update(id, formData);
      if (response.success) {
        navigate(`/plans/${id}`);
      }
    } catch (err) {
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500">Modify subscription plan settings</p>
              </div>
            </div>
          </div>
        </div>
              ...
              {features.length === 0 ? (
                ...
                <div className="space-y-2">
                  {features.map(feature => {
                    const selectedFeature = formData.features.find(f => (f.feature?._id || f.feature) === feature._id);
                    return (
