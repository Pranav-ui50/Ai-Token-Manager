 ...
 *
 * SECURITY: Sensitive credentials are loaded using the credentials service
 * which supports encrypted values (*_ENCRYPTED) for enhanced security.
 */
...
if (config.nodeEnv === 'production') {
  ...
  if (missing.length > 0) {
    ...
    console.error('Set these variables or use encrypted versions (*_ENCRYPTED)');
    process.exit(1);
  }
  ...
  for (const cred of plaintextCredentials) {
    if (process.env[cred] && !process.env[`${cred}_ENCRYPTED`]) {
      console.warn(`⚠️  Warning: ${cred} is stored as plaintext. Consider using ${cred}_ENCRYPTED for better security.`);
    }
  }
}
